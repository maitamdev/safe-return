...
export default function SubmitClaimPage() {
  ...
  if (loadingBounties && !stableMeta) {
    return <div className="app-card mx-auto max-w-2xl p-6 text-sm text-ink-soft">Đang tải dữ liệu tin...</div>;
  }
  ...
  const submit = async () => {
    setError(null);
    if (!connected) {
      setError("Hãy kết nối Phantom và chọn mạng thử nghiệm (Devnet).");
      return;
    }
    ...
    return (
      <div className="mx-auto max-w-4xl">
        <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div><h1 className="text-2xl font-bold">Hồ sơ tìm thấy đã được ghi nhận</h1><p className="mt-2 text-sm text-ink-soft">Chủ đồ sẽ xem kết quả và quyết định trả thưởng.</p></div>
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <div className="alert-box-warn mt-5 flex items-start gap-3 rounded-xl p-4 text-sm leading-6">
        ...
        <p>Không đăng số điện thoại, email, số giấy tờ hoặc chi tiết bí mật. Nội dung, địa điểm, thời gian và ảnh chỉ mở cho bạn, chủ tin và trọng tài được phân công; mạng chỉ lưu mã kiểm tra bằng chứng.</p>
      </div>

      {!connected && <div className="app-card mt-6 p-5"><p className="mb-3 text-sm text-ink-soft">Kết nối ví trên mạng thử nghiệm để gửi hồ sơ.</p><ConnectWalletButton size="md" /></div>}
      ...
      <div className="app-card mt-6 space-y-5 p-5 sm:p-7">
        ...
        <button type="button" disabled={busy || txState === "pending" || !connected} onClick={() => void submit()} className="app-button-primary w-full">{busy ? "Đang gửi bằng chứng..." : "Gửi bằng chứng riêng tư"}</button>
      </div>
    </div>
  );
}
...
function EmptyMessage({ id }: { id: string }) {
  return <div className="app-card mx-auto max-w-lg p-6 text-center"><h1 className="text-xl font-bold">Không tìm thấy tin</h1><p className="mt-2 text-sm text-ink-soft">Tin {id || "này"} không tồn tại hoặc bạn không có quyền xem.</p><Link href="/bounties" className="app-button-primary mt-5">Về danh sách</Link></div>;
}