  ...
  return (
    <div className="min-h-[100dvh] bg-bg text-ink">
      <header className="sticky top-0 z-40 border-b border-line bg-white/94 backdrop-blur-xl">
        <div className="mx-auto flex h-[4.5rem] max-w-7xl items-center justify-between gap-3 px-4 sm:px-6 lg:px-8">
          ...
          <nav className="hidden items-center gap-1 md:flex" aria-label="Điều hướng chính">
            {nav.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} aria-current={active ? "page" : undefined} className={cn("flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold transition", active ? "bg-mint-soft text-forest" : "text-ink-soft hover:bg-bg-deep hover:text-ink")}>
        ...
        <div className="flex items-center justify-between gap-3 border-t border-line bg-[#fbfcfb] px-4 py-2 xl:hidden">
          <NetworkBadge showBalance={false} />
          <TokenBalances />
        </div>
      </header>
      ...
      {(error || txState === "pending" || (txState === "confirmed" && lastTxUrl)) && (
        <div className={cn("border-b", txState === "failed" ? "border-rose-200 bg-rose-50" : txState === "confirmed" ? "border-emerald-200 bg-emerald-50" : "border-line bg-white")} role={txState === "failed" ? "alert" : "status"}>
          <div className="mx-auto flex max-w-7xl items-start gap-2 px-4 py-3 text-sm sm:px-6 lg:px-8">
            {txState === "pending" && <><span className="mt-1 h-2 w-2 animate-pulse rounded-full bg-forest" /><span className="text-ink">Đang gửi {transactionLabel(lastIx)}. Kiểm tra cửa sổ Phantom để ký.</span></>}
            {txState === "confirmed" && lastTxUrl && <a href={lastTxUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 font-semibold text-forest hover:underline"><CheckCircle size={17} weight="fill" />Giao dịch thành công. Mở Solana Explorer</a>}
            {txState === "failed" && error && <><WarningCircle size={18} className="mt-0.5 shrink-0 text-rose-700" /><span className="min-w-0 flex-1 text-rose-900">{error}</span><button type="button" onClick={clearError} className="rounded-lg p-1 text-rose-700 hover:bg-rose-100" aria-label="Đóng thông báo"><X size={16} /></button></>}
          </div>
        </div>
      )}

      <main className="mx-auto max-w-7xl px-4 py-6 pb-24 sm:px-6 md:py-10 lg:px-8 lg:pb-12">{children}</main>

      <nav className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-5 border-t border-line bg-white/98 px-1 py-1.5 shadow-[0_-8px_30px_rgba(28,52,41,0.08)] backdrop-blur-lg md:hidden" aria-label="Điều hướng di động">
        {nav.map((item) => {
